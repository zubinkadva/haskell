...
solve r c h w grid = 
 if r == h + 1 || c == w + 1 then numTriangles grid
 else f1 r c h w grid

f1 r c h w grid = if grid !! r !! c == black then 
 if c == w + 1 then solve(r + 1) 1 h w grid else solve r (c + 1) h w grid
 else f2 r c h w grid
 
f2 r c h w grid = solve r c h w [[x | x <- [white..sw]]]
